...
const getApiKey = () => {
  ...
  const localKey =
    typeof window !== "undefined"
      ? localStorage.getItem("healthai_api_key")
      : null;
...
const CONDITIONS_DATABASE: Record<
  string,
  { conditions: string[]; urgency: "low" | "medium" | "high" | "critical" }
> = {
  headache: {
...
- LOW: Self-care appropriate, monitor symptoms`;
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          recommendedAction:
            result.recommendedAction || "Consult a healthcare provider",
...
function fallbackSymptomAnalysis(
  ...
  if (
    criticalKeywords.some((keyword) =>
      additionalInfo.toLowerCase().includes(keyword),
    )
  ) {
    maxUrgency = "critical";
  }
