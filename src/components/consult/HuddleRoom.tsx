...
export function HuddleRoom({
        ...
        case "doctor_activated":
          ...
          setOrbState((prev) => {
            ...
            if (!exists)
              return [
                ...prev,
                { role: event.role as DoctorRole, status: "active" },
              ];
            return prev.map((o) =>
              o.role === event.role ? { ...o, status: "active" } : o,
            );
          });
          setLiveFeedText(
            `${leadNames[event.role] ?? event.role} is reviewing...`,
          );
          emitDebugState({
            leadSwarms: {
              ...swarmDebugRef.current.leadSwarms,
              [event.role]:
                swarmDebugRef.current.leadSwarms?.[event.role as DoctorRole] ??
                ({
                  status: "running",
                  hypotheses: [],
                  residentDebate: [],
              ...
              [event.role]: {
                ...
                hypotheses: [
                  ...
                  ]?.hypotheses ?? []),
                  {
                    ...
                    reasoning: "",
                    residentRole: event.residentRole,
                  },
                ],
              } as SwarmLeadState,
            },
          ...
          emitDebugState({
            leadSwarms: {
              ...
              [event.role]: {
                ...
                status: "complete",
                rectification: {
                  doctorRole: event.role as DoctorRole,
  ...
  const primaryLead =
    allRoles.find((r) => agents[r]?.state === "done") ?? allRoles[0];
