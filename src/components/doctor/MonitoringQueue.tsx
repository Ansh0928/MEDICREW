  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {patients.length} patient{patients.length !== 1 ? "s" : ""} — sorted by
        urgency
      </p>
      {patients.map((patient) => {
        const urgencyInfo = URGENCY_BADGE[patient.effectiveUrgency] ?? {
          variant: "outline" as const,
          label: patient.effectiveUrgency,
        };
        const trendInfo =
          TREND_INDICATOR[patient.urgencyTrend] ??
              ...
              <div>
                ...
                <div className="mt-2">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className={`text-xs ${trendInfo.className}`}>
                      {trendInfo.label}
                    </span>
                  </div>
                </div>
                ...
                {patient.lastCheckIn ? (
                  <>
                    <p className="text-sm capitalize">
                      {patient.lastCheckIn.status}
                    </p>
                    {patient.lastCheckIn.response && (
                      <p
                        className={`text-sm font-medium capitalize ${RESPONSE_COLOR[patient.lastCheckIn.response] ?? ""}`}
                      >
                        {patient.lastCheckIn.response}
                      </p>
                    )}
                ...
                {patient.lastConsultation ? (
                  <>
                    ...
                    <div className="flex items-center gap-1 mt-1 text-xs text-muted-foreground">
                      <Clock className="w-3 h-3" />
                      {new Date(
                        patient.lastConsultation.createdAt,
                      ).toLocaleDateString("en-AU")}
                    </div>
                  </>
                ) : (
                ...
                {patient.lastAgentActivity ? (
                  <>
                    <p className="text-xs font-medium text-blue-600">
                      {patient.lastAgentActivity.agentName}
                    </p>
                    <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">
                      {patient.lastAgentActivity.message}
                    </p>
                    <div className="flex items-center gap-1 mt-1 text-xs text-muted-foreground">
                      <Clock className="w-3 h-3" />
                      {new Date(
                        patient.lastAgentActivity.updatedAt,
                      ).toLocaleDateString("en-AU")}
                    </div>
                  </>
                ) : (
