...
export function Entropy({ className = "", size = 400 }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    function animate() {
      safeCtx.clearRect(0, 0, size, size)
      ...
      particles.forEach(particle => {
        ...
        particle.neighbors.forEach(neighbor => {
          ...
          if (distance < 50) {
            ...
            safeCtx.strokeStyle = `${particleColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`
            safeCtx.beginPath()
            safeCtx.moveTo(particle.x, particle.y)
            safeCtx.lineTo(neighbor.x, neighbor.y)
            safeCtx.stroke()
          }
        })
      })
      safeCtx.strokeStyle = `${particleColor}4D`
      safeCtx.lineWidth = 0.5
      safeCtx.beginPath()
      safeCtx.moveTo(size / 2, 0)
      safeCtx.lineTo(size / 2, size)
      safeCtx.stroke()
      time++
