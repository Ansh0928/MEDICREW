...
export function buildSymptomsFromAnswers(answers: IntakeAnswer[]): string {
  const parts = answers
    .filter((a) => a.answer.trim().length > 0 && a.questionId !== INTAKE_QUESTION_IDS.confirm)
    .map((a) => `${a.question}: ${a.answer}`);
  const confirm = answers.find((a) => a.questionId === INTAKE_QUESTION_IDS.confirm);
...
export function buildHistorySummaryFromAnswers(answers: IntakeAnswer[]): string {
  ...
  const parts = answers
    ...
    .map((a) => {
      if (a.questionId === INTAKE_QUESTION_IDS.severity) return `Severity: ${a.answer}/10`;
