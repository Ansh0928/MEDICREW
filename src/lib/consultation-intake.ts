...
export type ConsultationPatientInfo = z.infer<typeof ConsultationPatientInfoSchema>;

export const ConsultationInputSchema = z.object({
  symptoms: z.string().trim().min(1, "Symptoms are required").max(4000, "Symptoms must be under 4000 characters"),
...
export function resolveConsultationPatientInfo(
  ...
  const candidate = {
    age: override?.age ?? (patientProfile?.age !== undefined && patientProfile?.age !== null ? String(patientProfile.age) : "unknown"),
    gender: override?.gender ?? patientProfile?.gender ?? "unknown",
    knownConditions: override?.knownConditions ?? patientProfile?.knownConditions ?? undefined,
...
export function buildPatientContext(info: ConsultationPatientInfo): string {
  const medications = info.medications?.length ? info.medications.join(", ") : "none";
