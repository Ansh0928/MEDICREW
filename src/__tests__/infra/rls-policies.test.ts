...
const migrationPath = join(process.cwd(), "prisma/migrations/0001_supabase_init/migration.sql");
...
describe("INFRA-02: Row Level Security", () => {
  ...
  test("migration SQL enables RLS on Consultation table", () => {
    expect(sql).toMatch(/ALTER TABLE.*"Consultation".*ENABLE ROW LEVEL SECURITY/i);
  });
  ...
  test("migration SQL enables RLS on Notification table", () => {
    expect(sql).toMatch(/ALTER TABLE.*"Notification".*ENABLE ROW LEVEL SECURITY/i);
  });

  test("migration SQL enables RLS on PatientConsent table", () => {
    expect(sql).toMatch(/ALTER TABLE.*"PatientConsent".*ENABLE ROW LEVEL SECURITY/i);
  });
