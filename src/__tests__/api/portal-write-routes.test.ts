...
vi.mock("@/lib/doctors-patients-store", () => ({
  ...
  getSymptomCheckById: vi
    .fn()
    .mockImplementation((id: string) =>
      id === "sc-1" ? mockCheck : undefined,
    ),
  updateSymptomCheckStatus: vi.fn(),
}));

vi.mock("@/agents/doctorConsultation", () => ({
  streamDoctorConsultation: vi.fn(async function* () {
    yield { type: "done" };
  }),
}));

vi.mock("@/lib/ai/doctors-patients-ai", () => ({
  generateDoctorInsights: vi
    .fn()
    .mockResolvedValue({ summary: "mild migraine" }),
...
describe("POST /api/portal/doctor-note", () => {
  ...
  it("rejects missing symptomCheckId", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/portal/doctor-note", {
        method: "POST",
        body: JSON.stringify({
          doctorId: "d1",
          doctorName: "Dr Jones",
...
describe("POST /api/portal/case-consult", () => {
  ...
  it("rejects missing symptomCheckId", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/portal/case-consult", {
        method: "POST",
        body: JSON.stringify({}),
        headers: { "Content-Type": "application/json" },
      }) as any,
...
describe("POST /api/portal/case-insights", () => {
  ...
  it("rejects missing symptomCheckId", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/portal/case-insights", {
        method: "POST",
        body: JSON.stringify({}),
        headers: { "Content-Type": "application/json" },
      }) as any,
