...
- [ ] **markAsRead silent failure** — When the PATCH to `/api/notifications` fails, the notification optimistically flips to "read" then snaps back via SWR revalidation, with no user-facing feedback. Add a toast or inline error so the patient knows the action failed. Injection point: `markAsRead()` catch block in `src/app/patient/page.tsx`. _(Patient portal UX — eng review 2026-04-08)_
- [ ] **Dashboard stale after new consultation** — SWR `revalidateOnFocus: false` means a patient who completes a consultation and returns to the portal without a page reload sees their old consultation list until they manually refresh. Consider adding a short `refreshInterval` (e.g. 60s) or a `revalidateOnMount: true` + router.refresh() call after consultation complete. _(Patient portal UX — eng review 2026-04-08)_
- [ ] **PatientProfilePanel shows wrong patient** — `PatientProfilePanel` in the doctor portal fetches `/api/patient/profile` (auth-scoped to the logged-in user) instead of the selected patient's profile. Needs new endpoint `/api/doctor/patients/[id]/profile` scoped to doctor's clinic. _(Doctor portal correctness — QA 2026-04-05)_
...
_Last updated: 2026-04-05 — QA run on production: 3 critical bugs fixed (demo login 502, doctor portal 403, PatientProfilePanel null crash), 2 issues deferred (PatientProfilePanel wrong patient P2, doctor sidebar 404s known)_
